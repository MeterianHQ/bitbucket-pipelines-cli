...
public class LocalGitClient {
    ...
    public LocalGitClient(String pathToRepo,
        ...
        try {
            ...
            checkAndMakeRemoteURISecure();
            currentBranch = getCurrentBranch();
            log.info(String.format("Workspace is pointing to branch %s (%s)", currentBranch, getCurrentBranchSHA()));
        } catch (Exception ex) {
            log.error("Error during initialisation of the git client", ex);
            throw new RuntimeException(ex);
        }
    }

    private void checkAndMakeRemoteURISecure() throws GitAPIException, URISyntaxException {
        String remoteURI = git.remoteList().call().get(0).getURIs().get(0).toPrivateASCIIString();
        remoteURI = remoteURI.replace("http:", "https:");
        RemoteSetUrlCommand remoteUriCommand = git.remoteSetUrl();
        remoteUriCommand.setName("origin");
        remoteUriCommand.setUri(new URIish(remoteURI));
        remoteUriCommand.call();
        remoteURI = git.remoteList().call().get(0).getURIs().get(0).toPrivateASCIIString();
        log.info(String.format("Current tracked repository: %s ", remoteURI));
    }
    ...
    public void applyCommitsToLocalRepo() throws GitAPIException, IOException {
            ...
            commitChanges(meterianBitbucketUser,
    ...
    private String getMeterianCommitMessage() {
        return String.format("Fixes applied via %s", meterianBitbucketUser);
    }
    ...
    public boolean currentBranchWasCreatedByMeterianClient() throws GitAPIException {
        ...
        if (iterator.hasNext()) {
            ...
            return author.getName().equalsIgnoreCase(meterianBitbucketUser) &&
                    author.getEmailAddress().equalsIgnoreCase(meterianBitbucketEmail);
        }
        return false;
    }
    ...
    private boolean byLocalFixedBranchName(Ref branch) {
        try {
            ...
            log.error("Error occurred trying to find a fixed-by-meterian branch", ex);
            throw new RuntimeException(ex);
        }
    }
}
